  ...
  methods: {
    ...
    async fetchAndValidateExamId(examId) {
      try {
        ...
        if (exam) {
          ...
          this.appHeader.examName =
            exam.exam_name || "CBT - MathHub Online Examination System";
    ...
    async fetchExamQuestions() {
      try {
        ...
        // If QuestionID exists in URL, open that question
    ...
    getQuestionIdFromURL() {
      ...
      if (
        ...
        questionNumber < 1 ||
        questionNumber > this.questionList.length
      ) {
        console.warn(`Invalid QuestionID: ${questionId}`);
        return;
      }

      // URL QuestionID is 1-based
      // Vue/JS array index is 0-based
    ...
    updateQuestionIdInURL() {
      const url = new URL(window.location.href);

      url.searchParams.set("QuestionID", String(this.currentQuestionIndex + 1));
